...
- 曲順の更新には1曲あたり50ユニットのAPIクォータを消費します（デフォルト上限: 1日10,000ユニット ≒ 約200件の更新まで無料）。
  Webアプリ版は保存時に**実際に動かす必要がある曲だけ**を更新するため、消費するのは「移動した曲数×50ユニット」です。
  たとえば100曲のプレイリストで1曲だけドラッグした場合は50ユニットで済みます（全曲を書き直すと5,000ユニット）。
...
曲一覧やプレイリスト名を他のツールに渡したいとき用に、JSONで書き出せます。

- **「一覧をコピー」**: 全プレイリストの名前・ID・曲数
- **「このプレイリストをコピー」**: 選択中のプレイリストの曲一覧（曲名・チャンネル名・videoId・playlistItemId）

押すとクリップボードにコピーされ、同時に画面にも表示されます。自動コピーが効かない環境では、表示された枠内を長押しして選択してください。読み込み済みの内容から組み立てるため、APIのクォータは消費しません。
